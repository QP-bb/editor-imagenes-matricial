...
function matrizAImagen(matriz, rutaSalida) {
  ...
  // 5. Asegurar que el directorio existe
...
function obtenerCanal(matriz, canal) {
  ...
  // 2. Crear matriz resultado (copia profunda)
...
function obtenerDimensionesImagen(rutaImagen) {
  // Leer el archivo PNG y obtener dimensiones
  const buffer = fs.readFileSync(rutaImagen);
  const png = PNG.sync.read(buffer);
...
function ajustarBrillo(matriz, factor) {
  // 1. Crear matriz resultado (copia profunda)
  const resultado = copiarMatriz(matriz);

  // 2. Para cada pixel multiplicar R, G y B por el factor
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
...
function convertirEscalaGrises(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      ...
      // 1. Calcular gris ponderado
      const gris =
        0.299 * pixel.r +
        0.587 * pixel.g +
        0.114 * pixel.b;

      const g = limitarValorColor(gris);
...
function aplicarSepia(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      ...
      const Rn = limitarValorColor(0.393 * r + 0.769 * g + 0.189 * b);
      const Gn = limitarValorColor(0.349 * r + 0.686 * g + 0.168 * b);
      const Bn = limitarValorColor(0.272 * r + 0.534 * g + 0.131 * b);
...
function detectarBordes(matriz, umbral = 50) {
  ...
  // Crear matriz resultado (blanco/negro)
  const resultado = [];

  for (let i = 0; i < filas; i++) {
    resultado[i] = [];
    for (let j = 0; j < columnas; j++) {
      // Por defecto, pixel = negro (no borde)
      let esBorde = false;

      const grisActual = grises[i][j].r; // r=g=b

      // 2. Comparar con vecino derecho e inferior
      if (j + 1 < columnas) {
        const dif = Math.abs(grisActual - grises[i][j + 1].r);
        if (dif > umbral) esBorde = true;
      }

      if (i + 1 < filas) {
        const dif = Math.abs(grisActual - grises[i + 1][j].r);
        if (dif > umbral) esBorde = true;
      }

      // Si es borde -> blanco, sino negro
